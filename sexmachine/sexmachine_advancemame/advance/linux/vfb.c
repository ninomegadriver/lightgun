...
long frame_draw_duration = 0;
long fb_data_size = 0;
int fb_ptr = 0;
long triggerTime = 0;

struct pi_timings hdmi_timings;
...
adv_error fb_mode_set(const fb_video_mode* mode)
{
	...
	// [SEXMACHINE] Passing framebuffer to globals vars
	fb_ptr = fb_state.ptr;
	fb_data_size = fb_state.fixinfo.smem_len;
	...
	opt = target_system(cmd);
	if (opt) {
		char tmp[500], dummy[100];
		sscanf(opt, "hdmi_timings=%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %ld %d",
		&hdmi_timings.h_active_pixels,
	  &hdmi_timings.h_sync_polarity,
	  &hdmi_timings.h_front_porch,
	  &hdmi_timings.h_sync_pulse,
	  &hdmi_timings.h_back_porch,
	  &hdmi_timings.v_active_lines,
	  &hdmi_timings.v_sync_polarity,
	  &hdmi_timings.v_front_porch,
	  &hdmi_timings.v_sync_pulse,
	  &hdmi_timings.v_back_porch,
	  &hdmi_timings.v_sync_offset_a,
	  &hdmi_timings.v_sync_offset_b,
	  &hdmi_timings.pixel_rep,
	  &hdmi_timings.frame_rate,
...
static adv_error fb_wait_vsync_ext(void)
{
	...
	if (ioctl(fb_state.fd, FBIO_WAITFORVSYNC, 0) != 0) {
		printf("This version of advmame requires VSYNC to work\n");
		exit(0);
...
// [SEXMACHINE] "MAP" - arduino like helper funcion..
